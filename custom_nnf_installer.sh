...
sudo chown -R ubuntu:ubuntu ~/git/NNF
deactivate
cd ~
printf "[Unit]\nDescription=Flask Web Application Server using Gunicorn\nAfter=network.target\n\n[Service]\nUser=ubuntu\nGroup=ubuntu\nWorkingDirectory=/home/ubuntu/git/NNF\nEnvironment="PATH=/home/ubuntu/git/NNF/menv/bin"\nExecStart=sudo /bin/bash -c 'mkdir -p /tmp/my-server; source /home/ubuntu/git/NNF/menv/bin/activate; gunicorn -w 1 -k gthread --thread=8 --bind unix:/tmp/my-server/ipc.sock app:app --preload --error-logfile /home/ubuntu/git/NNF/log.log --capture-output --log-level debug;'\n\nRestart=always\n\n[Install]\nWantedBy=multi-user.target\n" | sudo tee /etc/systemd/system/my-server.service
mkdir -p /tmp/my-server
printf "server {\nlisten 80;\nserver_name $deviceipaddress;\n\nlocation / {\nproxy_buffering off;\nproxy_cache off;\nproxy_set_header Connection '';\nproxy_http_version 1.1;\nchunked_transfer_encoding off;\ninclude proxy_params;\nproxy_pass http://unix:/tmp/my-server/ipc.sock;\n}\n}" | sudo tee /etc/nginx/sites-available/my-server
sudo nginx -t
